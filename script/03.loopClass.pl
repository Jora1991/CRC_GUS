...
die"usage:perl $0 <in:clustalO> <in:stat of GUS> <out>\n" unless @ARGV == 3;
my($clustalO,$stat,$out)=@ARGV;
...
foreach my $id (sort keys %reference){
    print "$id\t$reference{$id}\t$id2class{$id}\n";
}
