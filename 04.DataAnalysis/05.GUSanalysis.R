...
#
# ######################################################################
setwd("/Users/chenjunru/Documents/github/")
library(dplyr)
library(ggplot2)
...
##### analysis for the five multi-cohorts #####
groupMuti <- read.table("00.rawdata/Cohorts/Cohorts.661s.group.txt",sep = "\t",header = T) %>% dplyr::mutate(Stage = ifelse(Group == 'CTR','CTR',ifelse(Stage %in% c('S3','S4'),'S34',Stage)))

cohortResult <- list()
for (midCountry in c('AUS','FRA','GER')) {
  groupMuti <- read.table("00.rawdata/Cohorts/Cohorts.661s.group.txt",sep = "\t",header = T) %>% dplyr::mutate(Stage = ifelse(Group == 'CTR','CTR',ifelse(Stage %in% c('S3','S4'),'S34',Stage))) %>% dplyr::filter(Country == midCountry) %>% dplyr::filter(Stage != 'NA') %>% tibble::column_to_rownames('Sample_ID')
...
summary(aov(Osc_bacterium.GUS6~Stage + Gender, data=speciesAbun))
summary(glm(Stage~Gender+Osc_bacterium.GUS6, data=speciesAbun,family=binomial))
summary(aov(Par_chongii.GUS1~Stage + Gender, data=speciesAbun))
summary(glm(Stage~Gender+Par_chongii.GUS1, data=speciesAbun,family=binomial))
...
speciesAbun$Gender <- factor(as.vector(speciesAbun$Gender),levels = c('F','M'))
summary(aov(Par_chongii.GUS7~Stage + Gender, data=speciesAbun))
summary(glm(Stage~Gender+Par_chongii.GUS7, data=speciesAbun,family=binomial))
summary(aov(Stu_stutzeri.GUS~Stage + Alcohol, data=speciesAbun))
summary(glm(Stage~Alcohol+Stu_stutzeri.GUS, data=speciesAbun,family=binomial))
...
summary(glm(Stage~Age + Gender+Lac_eligens.GUS2, data=speciesAbun,family=binomial))
