...
mid.data %>% ggplot(aes(Group,value,fill=Group)) + geom_violin() + geom_boxplot(col='black',size=1,width =0.1,outlier.colour = NA) + scale_fill_manual(values = analysis.list.info$colors$Stage)+ theme_classic() + facet_wrap("variable",nrow = 1,ncol = 7,scales = "free") + geom_signif(comparisons = analysis.list.info$compaired$Stage, color="black",step_increase = 0.08, map_signif_level=function(p) if(p < 0.01){'+'}else{if(p < 0.05){'*'}else{sprintf("%.2f",p)}}, test = wilcox.test) + xlab("") + ylab("Abundance") + theme(axis.text.x = element_text(angle = 45,hjust = 1),legend.position = "none")
