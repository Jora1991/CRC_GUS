...
alpha.result %>% tibble::rownames_to_column('ID') %>% dplyr::mutate(Group=group[ID,'Stage']) %>% dplyr::filter(Group != 'NA') %>% ggplot(aes(Group,No.Of.BGUS,fill=Group)) + geom_boxplot() + theme_classic() + xlab('') + ylab('Number of gmGUS') + scale_fill_manual(values = analysis.list.info$colors$Stage) + theme(legend.position = "null") + geom_signif(comparisons = analysis.list.info$compaired$Stage, color="black",step_increase = 0.08, map_signif_level=function(p) if(p < 0.01){'+'}else{if(p < 0.05){'*'}else{sprintf("%.2f",p)}}, test = wilcox.test) + xlab("") + ylab("Number of gmGUS") + theme(axis.text.x = element_text(angle = 45,hjust = 1),legend.position = "none") + xlim(analysis.list.info$levels$Stage)
...
alpha.result %>% dplyr::select(Abundance.Of.BGUS,Stage) %>% ggplot(aes(Stage,Abundance.Of.BGUS,fill=Stage)) + geom_boxplot() + theme_classic() + xlab('') + ylab('Abundance of gmGUS') + scale_fill_manual(values = analysis.list.info$colors$Stage) + theme(legend.position = "null")  + geom_signif(comparisons = analysis.list.info$compaired$Stage, color="black",step_increase = 0.08, map_signif_level=function(p) if(p < 0.01){'+'}else{if(p < 0.05){'*'}else{sprintf("%.2f",p)}}, test = wilcox.test) + xlab("") + ylab("Abundance of gmGUS") + theme(axis.text.x = element_text(angle = 45,hjust = 1),legend.position = "none") + xlim(analysis.list.info$levels$Stage)
